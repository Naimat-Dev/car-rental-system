...
export const up = function(knex) {
    return knex.schema.createTable("carSpecifications", (table) => {
        ...
        table.enu('fuelType', ['petrol', 'diesel', 'electric', 'hybrid']).notNullable().defaultTo("petrol"); // Create a 'fuelType' column with enum values 'petrol', 'diesel', 'electric', or 'hybrid'; default value is 'petrol'
        ...
        table.foreign('carId').references('id').inTable('cars').onDelete('CASCADE'); // If a record in the 'cars' table is deleted, the associated records in 'carDetails' will also be deleted (CASCADE)
