...
export const up = function (knex) {
	...
	return knex.schema.createTable("blog", (table) => {
		...
		table.string("title").notNullable().comment("Title is required"); //title column with type string
		table.text("content").notNullable().comment("Content is required"); //content column with type text
		table.string("author").notNullable().comment("Author is required"); //author column with type string
