...
export const getCardsJoin = catchAsync(async (req, res, next) => {
    ...
    const cardsWithoutSensitiveData = cards.map(({ password, passwordResetToken, passwordResetExpires, cvv, ...rest }) => rest);
...
export const getCardByIdJoin = catchAsync(async (req, res, next) => {
    ...
    // Remove sensitive fields such as CVV, password, etc.
    const { password, passwordResetToken, passwordResetExpires, cvv, ...cardWithoutSensitiveData } = card;
